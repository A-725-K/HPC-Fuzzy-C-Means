		...
		for (int k=0; k<N_CL; k++) {
			...
			for (int i=0; i<DIM; i++) {
				for (int j=myid*(SIZE/P); j<(myid+1)*(SIZE/P); j++) {
					double sum_den = 0.0;
					double u_ij = pow(mv[j*N_CL+k], M) * X[j].dims[i];
					
					MPI_Allreduce(&u_ij, &(sum_dims[i]), 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    for (int i=myid*(SIZE/P); i<(myid+1)*(SIZE/P); i++)
        for (int j=0; j<N_CL; j++) {
            double local_J = pow(mv[i*N_CL + j], M) * pow(distance(X[i], cc[j]), 2);
						MPI_Allreduce(&local_J, &global_J, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
				}
    return global_J;
}
...
void fuzzyCMeans(point *X, int *Y, int P, int myid) {
    ...
    for (n=0; n<N_ITER; n++) {
        ...
        for (int i=myid*(SIZE/P); i<(myid+1)*(n/P); i++)
            for (int j=0; j<N_CL; j++)
